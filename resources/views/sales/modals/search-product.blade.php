...
<div x-show="isSearchModal" 
     x-cloak
     @keydown.escape.window="isSearchModal = false; setTimeout(() => { searchQuery = ''; productosFiltrados = []; selectedIndex = -1; }, 200);"
    ...
    <div x-show="isSearchModal"
         ...
         @click.stop
         @mousedown="if ($event.target === $el || ($event.target.tagName !== 'BUTTON' && $event.target.tagName !== 'INPUT' && !$event.target.closest('button') && !$event.target.closest('input'))) { setTimeout(() => $refs.searchInputDesktop?.focus(), 0); }"
         class="hidden md:block no-scrollbar relative w-full max-w-6xl max-h-[90vh] overflow-hidden rounded-xl bg-white shadow-2xl dark:bg-gray-900 mx-4 flex flex-col modal-content">

        <!-- Botón cerrar -->
        <button @click="isSearchModal = false; setTimeout(() => { searchQuery = ''; productosFiltrados = []; }, 200);"
        ...
        <div class="px-4 sm:px-4 lg:px-4 pt-4 pb-4 border-b border-gray-200 dark:border-gray-700">
            <div class="relative w-full group">
                ...
                <input type="text" 
                       ...
                       x-model="searchQuery"
                       @input="filtrarProductos(); selectedIndex = -1;"
                       @keydown.arrow-down.prevent.stop="if (productosFiltrados && productosFiltrados.length > 0) { selectedIndex = selectedIndex < productosFiltrados.length - 1 ? selectedIndex + 1 : productosFiltrados.length - 1; scrollToSelected(); }"
                       @keydown.arrow-up.prevent.stop="if (productosFiltrados && productosFiltrados.length > 0) { selectedIndex = selectedIndex > 0 ? selectedIndex - 1 : 0; scrollToSelected(); }"
                       @keydown.enter.prevent.stop="if (selectedIndex >= 0 && productosFiltrados && productosFiltrados[selectedIndex]) { agregarProductoDesdeBusqueda(productosFiltrados[selectedIndex]); } else if (productosFiltrados && productosFiltrados.length === 1) { agregarProductoDesdeBusqueda(productosFiltrados[0]); }"
                       @focusout="setTimeout(() => { if (isSearchModal && (!$event.relatedTarget || (!$event.relatedTarget.closest('button') && $event.relatedTarget !== $refs.searchInputDesktop))) { $refs.searchInputDesktop?.focus(); } }, 150)"
                       autofocus
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400 sticky top-0">
                        <tr>
                            ...
                            <th scope="col" class="p-4 whitespace-nowrap">Descripción</th>
                    ...
                    <tbody>
                        ...
                        <template x-for="(producto, index) in productosFiltrados" :key="producto.id">
                            <tr :class="selectedIndex === index ? 'bg-blue-50 dark:bg-blue-900/20 border-b dark:border-gray-700' : 'bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700'"
                                ...
                                <th scope="row" class="px-4 py-3 font-medium text-gray-900 dark:text-white">
                                    <div class="flex flex-col gap-4 sm:flex-row sm:items-center">
                                        <div class="h-12.5 w-15 rounded-md">
                                            <img :src="producto.imagen" :alt="producto.nombre" class="h-full w-full object-cover rounded-md" />
                                        </div>
                                        <div class="flex flex-col flex-1 min-w-0">
                                            <div class="mb-1">
                                                <span class="text-base font-semibold whitespace-nowrap text-black dark:text-white block" x-text="producto.nombre"></span>
                                            </div>
                                            <div>
                                                <span class="text-xs text-gray-500 dark:text-gray-400 font-mono block" x-text="producto.codigo"></span>
                                            </div>
                                        </div>
                                    </div>
                                </th>
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <span class="text-sm font-mono text-gray-900 dark:text-white" x-text="producto.numero_lote || '-'"></span>
                                </td>
                                <td class="px-4 py-3">
                                    <span class="text-sm text-gray-900 dark:text-white" x-text="producto.descripcion || '-'"></span>
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <span class="block text-sm text-gray-900 dark:text-white" x-text="producto.stock || '-'"></span>
                                </td>
                                <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <button type="button" 
                                            @click="agregarProductoDesdeBusqueda(producto)"
    ...
    <div x-show="isSearchModal"
        ...
        <div x-show="isSearchModal"
            ...
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white">Buscar Productos</h3>
                </div>
                <button @click="isSearchModal = false; setTimeout(() => { searchQuery = ''; productosFiltrados = []; }, 200);"
        ...
        <div x-show="isSearchModal"
            ...
            <div class="space-y-3">
                <template x-for="(producto, index) in productosFiltrados" :key="producto.id">
                    <div :class="selectedIndex === index ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-300 dark:border-blue-700' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700'"
                         @mouseenter="selectedIndex = index"
                         :data-product-index="index"
                        ...
                        <div class="flex items-start gap-4 mb-4">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400 font-mono mb-1" x-text="producto.codigo"></p>
                                <p class="text-xs font-mono text-blue-600 dark:text-blue-400 font-semibold" x-text="producto.numero_lote || '-'"></p>
                            </div>
                        </div>
                        <div class="mb-4">
                            <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">Descripción</p>
                            <p class="text-sm text-gray-900 dark:text-white" x-text="producto.descripcion || '-'"></p>
                        </div>
                        <div class="mb-4">
                            <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">Stock</p>
                            <p class="text-sm font-semibold text-gray-900 dark:text-white" x-text="producto.stock || '-'"></p>
                        </div>
                        <button type="button" 
                                @click="agregarProductoDesdeBusqueda(producto)"
